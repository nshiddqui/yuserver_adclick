...
$settings = ['api' => 'api', 'apiVersion' => 'v1'];
$prefix = '/api/v1/';

function customUrl($routes, $api = '', $setting = [])
{
...
Router::scope('/', function (RouteBuilder $routes) use ($settings, $prefix) {
    ...
    $routes->connect('/start/*', ['controller' => 'Links', 'action' => 'start']);

    customUrl($routes);
    customUrl($routes, $prefix, $settings);

    Router::connect($prefix . '/:controller/:action/*', $settings);
    Router::connect($prefix . '/:controller/', ['action' => 'index'] + $settings);
